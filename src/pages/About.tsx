  ...
  return <Layout>
      ...
      <Section size="large">
        <div className="max-w-3xl mx-auto">
          <SectionHeader title="Where We Started" centered />
          <div className="mt-8 prose prose-lg prose-slate mx-auto font-body">
            <p className="leading-relaxed text-sidebar-primary">
      ...
      {/* Leadership Team */}
      <Section variant="muted">
        <SectionHeader title="Leadership" subtitle="Our team combines expertise in agricultural systems, food processing, and international markets." centered />
        <div className="mt-12 md:mt-16 grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {leadershipTeam.map((member, index) => <div key={member.name} className="bg-background p-6 rounded-lg shadow-soft animate-fade-in text-center" style={{
          animationDelay: `${index * 0.1}s`
        }}>
              <div className="w-20 h-20 mx-auto rounded-full bg-primary/10 flex items-center justify-center mb-4 overflow-hidden">
                {member.image ? <img src={member.image} alt={member.name} className="w-full h-full object-cover" /> : <User className="w-10 h-10 text-primary" />}
              </div>
      ...
      {/* Advisory Board */}
      <Section>
        <SectionHeader title="Advisory Board" subtitle="We are guided by experienced professionals who bring deep expertise across agriculture, finance, food science, and international trade." centered />
        <div className="mt-12 md:mt-16 grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {advisoryBoard.map((advisor, index) => <div key={advisor.name} className="border border-border p-6 rounded-lg animate-fade-in text-center" style={{
          animationDelay: `${index * 0.1}s`
        }}>
      ...
      {/* Values */}
      <Section>
        <SectionHeader title="How We Work" subtitle="Our approach is grounded in a few core principles that guide how we build, operate, and grow." centered />
        <div className="mt-12 md:mt-16 grid sm:grid-cols-2 gap-8">
          {values.map((value, index) => <div key={value.title} className="border border-border p-6 md:p-8 rounded-lg animate-fade-in" style={{
          animationDelay: `${index * 0.1}s`
        }}>
      ...
      <Section variant="muted">
        <div className="max-w-3xl mx-auto text-center">
          <SectionHeader title="Why Uganda" subtitle="Uganda offers a unique combination of factors that make it an ideal location for agricultural processing infrastructure." centered />
