  ...
  return <Layout>
        ...
        <div className="container relative z-10">
          <div className="max-w-2xl">
            ...
            delay: 0.2
              ...
              {hero?.subtitle || 'ATROUN develops integrated processing infrastructure to stabilize perishable produce, recover premium ingredients, and create circular material flows—connecting East African agriculture to global markets.'}
            </motion.p>
            <motion.div initial={{
            opacity: 0,
            y: 20
      ...
      <Section size="large">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          <div>
            <SectionHeader title={problemSection?.title || 'A Structural Challenge Becomes an Opportunity'} subtitle={problemSection?.subtitle || 'In Uganda, over 30% of perishable agricultural produce is lost after harvest due to limited processing capacity and cold-chain infrastructure. Meanwhile, global demand for shelf-stable, nutrient-preserving ingredients continues to grow.'} />
            <p className="mt-6 font-body leading-relaxed text-sidebar-primary">
              {problemSection?.body_content || 'ATROUN addresses this by bringing processing closer to the source. Our systems stabilize fresh produce through lyophilization (freeze-drying), extending shelf life from days to years—without refrigeration, without preservatives, and without compromising nutritional integrity.'}
            </p>
            <div className="mt-8">
          ...
          <div className="relative">
            <img src={problemSection?.image || productImage} alt={problemSection?.image_alt || 'Freeze-dried avocado ingredients'} className="rounded-lg shadow-elevated w-full aspect-square object-cover" />
      ...
      <Section variant="muted">
        ...
        <div className="mt-12 md:mt-16 grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          return <div key={item.title} className="bg-background p-6 rounded-lg shadow-soft animate-fade-in" style={{
      ...
      <Section>
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          <div className="order-2 lg:order-1">
            <img alt={techSection?.image_alt || 'ATROUN processing facility'} className="rounded-lg shadow-elevated w-full aspect-[4/3] object-cover" src={techSection?.image || '/lovable-uploads/47078f44-976a-40a1-863a-3bfcbe7c89d0.png'} />
          </div>
          <div className="order-1 lg:order-2">
            <SectionHeader title={techSection?.title || 'Technology Built for African Conditions'} subtitle={techSection?.subtitle || 'Our platform combines proven lyophilization technology with modular, scalable design principles suited to the realities of East African agriculture.'} />
            <ul className="mt-8 space-y-4">
              {(techSection?.body_content ? techSection.body_content.split('\n').filter(Boolean) : ['Feedstock valorization across multiple crop categories', 'Controlled processing environments with quality documentation', 'Ingredient recovery and fractionation for diverse end markets', 'Biochar production from organic residues']).map(item => <li key={item} className="flex items-start gap-3">
      ...
      <Section variant="primary" size="large">
        <div className="text-center max-w-3xl mx-auto">
          ...
          <p className="mt-6 text-lg md:text-xl font-body text-primary-foreground/80 leading-relaxed">
            {ctaBlock?.subtitle || "ATROUN is designed as a scalable, replicable model—not a single-project venture. We're building infrastructure that can grow with African agriculture and meet evolving global market demands."}
          </p>
